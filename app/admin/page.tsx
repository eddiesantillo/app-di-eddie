      ...
      {view === 'calendario' && (
        <div>
          {(data?.calendario || []).map((c: any, i: number) => (
            <div key={i} style={{marginBottom: '20px', border: '1px solid #555', padding: '15px', background: '#222', borderRadius: '8px'}}>
              <input value={c.title || ''} onChange={(e) => { const n = [...(data.calendario || [])]; n[i] = {...n[i], title: e.target.value}; setData({...data, calendario: n}); }} style={inputStyle} placeholder="Nome Concerto" />
              <input type="date" value={c.start || ''} onChange={(e) => { const n = [...(data.calendario || [])]; n[i] = {...n[i], start: e.target.value}; setData({...data, calendario: n}); }} style={inputStyle} />
              <input value={c.location || ''} onChange={(e) => { const n = [...(data.calendario || [])]; n[i] = {...n[i], location: e.target.value}; setData({...data, calendario: n}); }} style={inputStyle} placeholder="Luogo" />
              <select value={c.tipo || 'RL'} onChange={(e) => { const n = [...(data.calendario || [])]; n[i] = {...n[i], tipo: e.target.value}; setData({...data, calendario: n}); }} style={inputStyle}>
                <option value="RL">RL</option>
                <option value="SL">SL</option>
              </select>
              <input value={c.link || ''} onChange={(e) => { const n = [...(data.calendario || [])]; n[i] = {...n[i], link: e.target.value}; setData({...data, calendario: n}); }} style={inputStyle} placeholder="Link (URL)" />
