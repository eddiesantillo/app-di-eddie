    ...
    <main style={{ padding: '40px 20px', color: '#fff', maxWidth: '800px', margin: '0 auto', fontFamily: 'sans-serif' }}>
      ...
      {!activeSection ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
          {['bio', 'radio', 'calendario', 'social', 'shop', 'foto', 'repertorio'].map(id => (
        ...
        <div style={cardStyle}>
          ...
          {/* GESTIONE REPERTORIO - NUOVA */}
          {activeSection === 'repertorio' && (
            <div>
              <h2>Gestione Repertorio</h2>
              <p style={{color: '#888', fontSize: '0.9em', marginBottom: '10px'}}>Incolla qui tutto l'elenco (titolo e autore):</p>
              <textarea 
                value={data.repertorio || ''} 
                onChange={(e) => setData({...data, repertorio: e.target.value})} 
                style={{...inputStyle, height: '400px', whiteSpace: 'pre'}} 
                placeholder="Esempio:&#10;Canzone 1 - Autore 1&#10;Canzone 2 - Autore 2"
              />
            </div>
          )}
          ...
          {activeSection === 'foto' && (
            <div>
              ...
              <div style={{ marginBottom: '20px', padding: '15px', background: '#262626', borderRadius: '8px', border: '1px dashed #444' }}>
                <label style={{ cursor: 'pointer', display: 'block', textAlign: 'center' }}>
                  <input type="file" accept="image/*" onChange={handleFileChange} style={{ display: 'none' }} />
                  <span style={{ color: '#28a745', fontWeight: 'bold' }}>+ Seleziona Foto</span>
                </label>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
                {fotoList.map((f, i) => (
                  <div key={i} style={{ position: 'relative' }}>
                    <img src={f.src} alt="Foto" style={{ width: '100%', height: '150px', objectFit: 'cover', borderRadius: '8px', border: '1px solid #444' }} />
                    <button onClick={() => deleteFoto(f.id)} style={{ position: 'absolute', top: '5px', right: '5px', background: '#721c24', color: '#fff', borderRadius: '50%', border: 'none', padding: '5px 10px', cursor: 'pointer' }}>X</button>
                  </div>
                ))}
              </div>
            </div>
